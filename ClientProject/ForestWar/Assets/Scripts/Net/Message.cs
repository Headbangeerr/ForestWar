...
public class Message  {
    ...
    public void ReadMessage(int newDataAmount, Action<ActionCode, string> processDataCallBack)
    {
        ...
        while (true)//循环处理数据
        {
            ...
            {
                ActionCode actionCode = (ActionCode)BitConverter.ToInt32(data, 4);//通过强转，从秩为4的位置获取RequestCode                
                string dataStr = Encoding.UTF8.GetString(data, 8, dataLength - 4);//从秩为8的位置开始获取请求中的数据，这里减去4是因为数据中的前4
                //Debug.Log("接收到["+actionCode.ToString()+"]类型的响应：" + dataStr);
                processDataCallBack(actionCode, dataStr);//通过回调函数对解析出来的请求数据作进一步的处理
    ...
    public static byte[] PackData(RequestCode requestCode,ActionCode actionCode, string data)
    {
        ...
        int dataLength = dataBytes.Length + requestCodeBytes.Length+actionCodeBytes.Length;//计算数据包中的数据总长度
        byte[] dataLengthBytes = BitConverter.GetBytes(dataLength);
        //将转换后的数据按照数据包：【数据总长度+RequestCode+ActionCode+数据】的格式拼接
        byte[] packageBytes = dataLengthBytes.Concat(requestCodeBytes).Concat(actionCodeBytes).Concat(dataBytes).ToArray();
        return packageBytes;
    }
}
